...
def main():
    ...
    # Check if the camera/video is successfully opened
    ...
    while True:
        ...
        if not grabbed:
            print("[INFO] End of video or webcam feed")
            break

        # if the ROI has been computed, track it using CamShift
        if roiBox is not None:
            ...
            # apply CamShift to get the new tracking window
            (r, roiBox) = cv2.CamShift(backProj, roiBox, termination)
            pts = np.int32(cv2.boxPoints(r))  # Use np.int32 instead of np.int0
            cv2.polylines(frame, [pts], True, (0, 255, 0), 2)

        # show the frame
        cv2.imshow("frame", frame)
        key = cv2.waitKey(10) & 0xFF  # Give a bit more delay for key processing
        ...
        if key == ord("i") and len(roiPts) < 4:
            ...
            print("[INFO] ROI selection mode: click 4 points.")
            ...
            # compute the HSV histogram and normalize it
            roiHist = cv2.calcHist([roi], [0], None, [16], [0, 180])
            roiHist = cv2.normalize(roiHist, roiHist, 0, 255, cv2.NORM_MINMAX)
            roiBox = (tl[0], tl[1], br[0], br[1])
            print("[INFO] ROI selection complete.")
